...
# Local PVs sharing one disk filesystem will have same capacity and will have no capacity isolation. If you want to separate a disk into multiple PVs with capacity isolation. You can separate disk into multiple partitions first.


sudo mkfs.xfs /dev/vde
DISK_UUID=$(blkid -s UUID -o value /dev/vde) 
sudo mkdir /mnt/$DISK_UUID
sudo mount /dev/vde /mnt/$DISK_UUID
echo UUID=`sudo blkid -s UUID -o value /dev/vde` /mnt/$DISK_UUID xfs defaults 0 0 | sudo tee -a /etc/fstab
